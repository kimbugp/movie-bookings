...
class TestTickets(BaseTestCase):
    ...
    def test_create_cinema_with_no_permissions_fails(self, test_client):
        response = test_client.post("/api/v1/ticket")
        assert response.status_code == 401
    ...
    def test_get_all_ticket(self, test_client, auth_header):
        response = test_client.get("/api/v1/ticket", headers=auth_header)
    ...
    def test_filter_tickets_by_query_params(self, test_client, auth_header):
        ...
        self.assertKeys(
            ...
            {"payment_method": "mm", "user_id": 1, "price": 20000.0},
        )
        self.assertEqual(response.status_code, 200)
...
class TestNormalUserTickets(BaseTestCase):
    ...
    def test_create_bulk_ticket_fails_with_invalid_seats(
        ...
        self.assertKeys(
            response.json,
            {
                "message": "seat numbers '{6, 7, 8, 9, 10}' in cinema hall not available check available seats for showtime",
                "error": "error",
            },
        )
