...
class TestAuthentication(BaseTestCase):
    def test_registration_succeeds(self, test_client):
        data = json.dumps(
            {"email": "simonp@bb.com", "name": "string", "password": "string"}
        )
    ...
    def test_login_fails_with_non_existing_user(self, test_client):
        data = json.dumps({"email": "invalid@bb.com", "password": "dsfdsf"})
        response = test_client.post(
    ...
    def test_get_current_user(self, auth_header, test_client):
        response = test_client.get("/api/v1/auth", headers=auth_header)
        self.assertEqual(response.status_code, 200)
...
class TestUserFiltering(BaseTestCase):
    ...
    def test_get_user_greater_than_10000(self, auth_header, test_client):
        response = test_client.get(
            "/api/v1/users?ticket_startdate__gt=2017-01-11&ticket_enddate__lt=2019-10-11&total=100000&report=True",
            headers=auth_header,
        )
        self.assertEqual(response.status_code, 200)

    def test_get_user_reports(self, auth_header, test_client):
        response = test_client.get("/api/v1/users?report=True", headers=auth_header)
        self.assertEqual(response.status_code, 200)
