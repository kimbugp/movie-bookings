...
class TestShowTime(BaseTestCase):
    ...
    def test_create_showtime_succeeds(self, showtime):
        ...
        self.assertEqual(
            response.json["showtimes"][0]["show_datetime"],
            "2019-11-10 00:00:00",
        )

    def test_create_showtime_fails_with_not_found_args(
    ...
    def test_update_show_time_by_id_succeeds(
        self, test_client, showtime, auth_header
    ):
        data = json.dumps(
            {
...
class TestShowTimeUsers(BaseTestCase):
    def test_create_showtime_fails_when_not_admin(
